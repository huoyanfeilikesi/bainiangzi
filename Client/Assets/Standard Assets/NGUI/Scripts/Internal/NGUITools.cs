...
//#endif
    ...
    //自动设置子panel的depth
    static public void SetSubPanelDepthDeep(GameObject go)
    {
        ...
        do
        {
            ...
            for (int i = 0; i < transforms.Count; i++)
            {
                ...
                for (int j = 0; j < t.childCount; j++)
                {
                    ...
                    UIPanel findPanel = child.gameObject.GetComponent<UIPanel>();
                    if (findPanel != null)
                    {
                        findPanel.depth = baseDepth + (addDepth++);
                    }
                    temp.Add(child);
                }
            }
	...
	//自动设置子widget的depth
	static public void SetSubWidgetDepthDeep(GameObject go)
	{
		...
		do
		{
			...
			for (int i = 0; i < transforms.Count; i++)
			{
				...
				{
					Transform child = t.GetChild(j);
					UIWidget findWidget = child.gameObject.GetComponent<UIWidget>();
					if (findWidget != null)
					{
						findWidget.depth = baseDepth + (addDepth++);
					}
					temp.Add(child);
				}
			}
	...
	static public void FitToRootSize(UIWidget wid, float iAspect)
	{
		var obj = UnityEngine.GameObject.Find("GameLoader/UIRoot");
		var root = obj.GetComponent(typeof(UIRoot)) as UIRoot;
		var w = UnityEngine.Screen.width * root.pixelSizeAdjustment;
		var h = UnityEngine.Screen.height * root.pixelSizeAdjustment;
		if (iAspect > (w / h))
		{
			w = h * iAspect;
		}
	...
	//	NGUITools.FitToRootScale(_view.InitBgTexture_UITexture, 1334, 750);
	static public void FitToRootScale(UIWidget wid, float iDesignW, float iDesignH)
	{
		var obj = UnityEngine.GameObject.Find("GameLoader/UIRoot");
		var root = obj.GetComponent(typeof(UIRoot)) as UIRoot;
		var w = UnityEngine.Screen.width * root.pixelSizeAdjustment;
		var h = UnityEngine.Screen.height * root.pixelSizeAdjustment;
		var scaleW = w / iDesignW;
		var scaleH = h / iDesignH;
		var scale = Math.Max(scaleW, scaleH);
		wid.transform.localScale = new Vector3(scale, scale, 1);
	}
}
