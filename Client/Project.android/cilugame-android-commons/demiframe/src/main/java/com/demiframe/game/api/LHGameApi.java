...
abstract class LHGameApi {
  ...
  */
    ...
    public void switchAccount(final Activity activity, final Object extObject) {
        activity.runOnUiThread(new Runnable() {
            public void run() {
                LHGameApi.this.createAdapter(activity).userManagerProxy().switchAccount(activity, extObject);
            }
        });
    }
    ...
    public void onExit(final Activity activity, final IExitCallback paramIExitCallback) {
        activity.runOnUiThread(new Runnable() {
            public void run() {
                LHGameApi.this.createAdapter(activity).exitProxy().onExit(activity, paramIExitCallback);
            }
        });
    }
